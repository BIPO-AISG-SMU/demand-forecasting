...
target_variable = conf_loader.get("constants*")["general"]["target_feature"]
...
class DataPreprocessing:
    ...
    def run_pipeline(self, merged_df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Check if "drop_columns" is in merged_df
        columns_exist = self.merged_df.columns.isin(conf_params["drop_columns"]).all()
        if columns_exist:
    ...
    def check_and_handle_missing_values(self) -> None:
        ...
        # non_target_columns = self.merged_df.drop(columns=target_variable, axis=1).columns

        # Get all columns
        all_columns = self.merged_df.columns

        # Iterate through columns to check for missing values
        for column in all_columns:
            # Impute 0 values in proxy_revenue with median
            if column == target_variable:
                # if np.issubdtype(self.merged_df[column].dtype, np.number):
                median_value = self.merged_df[column].median()
                self.merged_df[column].replace(0, median_value, inplace=True)
                logging.info(
                    f"Imputed zero values in column '{column}' with median value of {median_value}.{self.merged_df[column].dtype}"
                )
