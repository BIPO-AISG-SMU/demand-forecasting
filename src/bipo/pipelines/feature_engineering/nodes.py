...
def generate_lag(
    ...
    Args:
        data_preprocessed_outlet_partitions (Dict[str, pd.DataFrame]): Kedro IncrementalDataSet dictionary containing individual outlet related features dataframe as values with filename as identifier.
...
def merge_fold_and_generated_lag(
    ...
    Args:
        fold_based_outlet_partitioned_data (Dict[str, pd.DataFrame): Kedro IncrementalDataSet dictionary containing individual outlet related features dataframe as values with filename as identifier.
        generated_lags_dict (Dict[str, pd.DataFrame]): Kedro IncrementalDataSet dictionary containing outlet-based dataframe comprising of lagged features as values which are identified by its corresponding key in the dictionary.
...
def apply_feature_encoding_transform(
    ...
    params_dict: Dict[str, Any],
    feature_encoding_dict: Dict[str, Any],
) -> pd.DataFrame:
    """This function applies necessary encoding transform which includes one-hot encoding and categorical encoding. Can be extended to include encoding functions if defined under apply_feature_encoding_fit function.

    Args:
        partitions_dict (Dict): Kedro IncrementalDataSet dictionary containing individual outlet related features dataframe as values with filename as identifier
        params_dict (Dict): Dictionary referencing key-values from parameters.yml.
        feature_encoding_dict (Dict): Dictionary containing new/updated encoding parameters which will be used for applying necessary transforms.

    Raises:
        None

    Returns:
        Dict[str, Any]: Dictionary containing new/updated encoding parameters.
    ...
    for partition_id, partition_load_df in partitions_dict.items():
        ...
        if not ordinal_encoding_dict:
            ...
            df = ordinal_encoding_transform(
                df=partition_load_df,
                ordinal_columns_dict=ordinal_encoding_dict,
                ordinal_encoder=feature_encoding_dict[f"{fold_info}_{outlet_info}_ord"],
            )
        ...
        if not one_hot_encoding_col_list:
            ...
            # Extract fold and outlet information when doing encoding based on filename as represented by partition_id
            logger.info(
                f"Applied one-hot-encoding transform for each {fold_info} and {outlet_info}..."
            )
            df = one_hot_encoding_transform(
                df=df,
                ohe_column_list=one_hot_encoding_col_list,
                ohe_encoder=feature_encoding_dict[f"{fold_info}_{outlet_info}_ohe"],
            )
...
def apply_feature_encoding_fit(
    ...
    Returns:
        Dict[str, Any]: Dictionary containing new/updated encoding parameters encompassing sklearn onehotencoder and ordinalencoder. Other encodings can be included if they are implemented in this function.
    ...
    logger.info("Encoding fit completed.\n")
...
def apply_standard_norm_fit(
    ...
    if not columns_to_norm_list:
        ...
        logger.info(f"Standardised/Normalisation of data completed.\n")
...
def apply_standard_norm_transform(
    partitions_dict: Dict[str, pd.DataFrame],
    params_dict: Dict[str, Any],
    std_norm_encoding_dict: Dict[str, Any],
) -> Dict[str, pd.DataFrame]:
    """This function applies preferred standardise/normalisation as configured in parameters.yml.
    ...
    Args:
        partitions_dict (Dict[str, pd.DataFrame]): Dictionary containing file represented by fold based partitions of a selected data split source as defined in Kedro framework.
        params_dict (Dict[str, Any]): Dictionary referencing key-values from parameters.yml
        std_norm_dict (Dict[str, Any]): Dictionary containing new/updated encoding parameters.
...
def apply_binning_transform(
    ...
    if not binning_encoding_dict or not binning_col_dict:
        ...
        for partition_id, partition_df in partitions_dict.items():
            ...
            for bin_col, bin_labels in binning_col_dict.items():
                logger.info(
                    f"Extracting binning parameters through equal frequency binning"
                )
