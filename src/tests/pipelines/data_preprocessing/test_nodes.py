# import data_preprocessing nodes
...
def test_outlet_exclusion_list_check_correct_input():
    # Test for valid outlet exclusion list
    assert outlet_exclusion_list_check([1, 2, 3]) == {"1", "2", "3"}


def test_outlet_exclusion_list_check_incorrect_input(default_values):
...
def test_outlet_exclusion_list_check_correct_output_type():
...
def test_const_value_perc_check_incorrect_input(default_values):
...
def test_date_validity_check_correct_dates():
...
def test_date_validity_check_inverted_dates():
...
def test_date_validity_check_incorrect_format(default_values):
...
@pytest.fixture
def mock_non_revenue():
    # Fixture to mock non-revenue data
    df = pd.DataFrame(
        {"Date": ["2021-01-01", "2021-01-02", "2021-01-03"], "factor": [1.75, 1.5, 1.0]}
    )
...
@pytest.fixture
def mock_non_revenue_diff_columns():
...
@pytest.fixture
def mock_non_revenue_diff_dates():
    # Fixture to mock non-revenue data with differing dates
    df = pd.DataFrame(
        {"Date": ["2021-12-01", "2021-12-02", "2021-12-03"], "factor": [1.75, 1.5, 1.0]}
    )
    df.set_index(pd.to_datetime(df["Date"]), inplace=True)
    return df
...
def test_merge_non_proxy_revenue_data_multiple_dataframes(
    mock_non_revenue, mock_non_revenue_diff_columns
):
...
def test_merge_non_proxy_revenue_data_multiple_dataframes_diff_dates(
    mock_non_revenue_diff_columns, mock_non_revenue_diff_dates
):
...
def test_merge_outlet_and_other_df_feature_empty_revenue_partitioned(mock_non_revenue):
