...
class TestDataLoaderNode:
    ...
    ######### Test `rename_columns` ##############
    def test_rename_columns(self, create_preprocessing_datapoints):
    ...
    def test_merge_unique_csv_xlsx_df(
        self, create_preprocessing_datapoints
    ):  # data from unique_daily_records (holiday, covid data)
        sample_df_to_merge = create_preprocessing_datapoints(
            {
        ...
        sample_df_to_merge_wrong_id = create_preprocessing_datapoints(
            {
        ...
        sample_base_df = create_preprocessing_datapoints(
    ...
    def test_rename_merge_unique_csv_xlsx_df_col_index(
        self, create_preprocessing_datapoints
    ):  # any data. This function is called after merge_df.
        sample_df = create_preprocessing_datapoints(
            {
                "Date": pd.to_datetime(
                    ["2021-01-01", "2021-01-02"], format="%Y-%m-%d"
                ),
                "Value1": [10, 20],
                "Value2": [30, 40],
            }
    ...
    def test_load_and_partition_proxy_revenue_data(
        self, create_preprocessing_datapoints
    ):  # data from proxy reveune
        sample_proxy_reveune_df = create_preprocessing_datapoints(
            {
        ...
        missing_costcentre_proxy_reveune_df = create_preprocessing_datapoints(
            {
        ...
        outlet_part_dict = load_and_partition_proxy_revenue_data(
            sample_proxy_reveune_df
        )
    ...
    def test_load_and_structure_propensity_data(
        self, create_preprocessing_datapoints
    ):  # data from propensity
        sample_propensity_df = create_preprocessing_datapoints(
                {
                    "Date": [
                        "2022-01-01",
                        "2022-01-01",
    ...
    def test_load_and_structure_marketing_data(
        self, create_preprocessing_datapoints
    ):  # data from marketing
        # all sample df has a date range of 7 days
        sample_campaign1_df = create_preprocessing_datapoints(
            {
                "Name": ["campaign1"] * 7,
        ...
        sample_campaign2_df = create_preprocessing_datapoints(
            {
                "Name": ["campaign2"] * 7,
                "Date Start": ["2021-01-01"] * 7,
                "Date End": ["2021-01-07"] * 7,
        ...
        sample_campaign3_df = create_preprocessing_datapoints(
            {
                "Name": ["campaign3"] * 7,
    ...
    def test_load_and_structure_weather_data(
        self, create_preprocessing_datapoints
    ):  # data from weather
        # all sample df has a date range of 7 days
        sample_weather_df = create_preprocessing_datapoints(
            {
