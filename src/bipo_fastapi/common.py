...
def explain_ebm_inference(
    ...
    for row in range(len(model_input_df)):
        ...
        if output_type == "png":
            filepath = os.path.join(output_filepath, f"local_contribution_to_prediction_{date_list[row]}.png")   
            plotly_fig.write_image(
               filepath
            )
        elif output_type == "html":
            filepath = os.path.join(output_filepath, f"local_contribution_to_prediction_{date_list[row]}.html")   
