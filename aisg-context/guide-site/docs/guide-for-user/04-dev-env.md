...
=== "Linux/macOS"
    ...
    ```bash
    $ docker build \
        -t "registry.aisingapore.net/100e-bipo/vscode-server:0.1.0" \
        -f docker/bipo-vscode.Dockerfile \
        --platform linux/amd64 .
    $ docker push registry.aisingapore.net/100e-bipo/vscode-server:0.1.0
    ```
...
=== "Linux/macOS"
    ...
    $ polyaxon run -f aisg-context/polyaxon/polyaxonfiles/vscode-service.yml \
        -P DOCKER_IMAGE="registry.aisingapore.net/100e-bipo/vscode-server:0.1.0" \
...
`registry.aisingapore.net/100e-bipo/vscode-server:0.1.0` and to
...
=== "Linux/macOS"
    ...
    ```bash
    $ docker build \
        -t registry.aisingapore.net/100e-bipo/jupyter-server:0.1.0 \
        -f docker/bipo-jupyter.Dockerfile \
        --platform linux/amd64 .
    $ docker push registry.aisingapore.net/100e-bipo/jupyter-server:0.1.0
    ```
...
=== "Linux/macOS"
    ...
    $ polyaxon run -f aisg-context/polyaxon/polyaxonfiles/jupyter-service.yml \
        -P DOCKER_IMAGE="registry.aisingapore.net/100e-bipo/jupyter-server:0.1.0" \
        -P WORKING_DIR="/polyaxon-v1-data" \
        -p bipo-<YOUR_NAME>
    ```
