...
- Artifacts for specific runs will be uploaded to a directory with a
  ...
  `s3://bipo-artifacts/mlflow-tracking-server/<MLFLOW_EXPERIMENT_UUID>`.
...
=== "Linux/macOS"
    ...
    $ conda activate bipo
    $ cd src
    $ gunicorn bipo_fastapi.main:APP -b 0.0.0.0:8080 -w 4 -k uvicorn.workers.UvicornWorker
    ```
...
`src/bipo_fastapi/config.py`. This class contains
...
`src/bipo_fastapi/config.py`:
```python
...
class Settings(pydantic.BaseSettings):

    API_NAME: str = "bipo_fastapi"
...
=== "Linux/macOS"
    ...
    $ docker build \
        -t https://registry.aisingapore.net/harbor/projects/55/repositories/fastapi-server:0.1.0 \
        --build-arg PRED_MODEL_UUID="$PRED_MODEL_UUID" \
        -f docker/bipo-fastapi.Dockerfile \
...
=== "Linux/macOS"
    ...
    $ docker run --rm -p 8080:8080 \
        ...
        https://registry.aisingapore.net/harbor/projects/55/repositories/fastapi-server:0.1.0
    ```
...
```bash
$ docker push https://registry.aisingapore.net/harbor/projects/55/repositories/fastapi-server:0.1.0
```
